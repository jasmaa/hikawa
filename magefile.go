...
func (p BuildPlatform) godotPluginCSharedName(appPath string) string {
	...
	switch p.OS {
	case "windows":
		return fmt.Sprintf("libgodotgo-hikawa-windows-4.0-%s.dll", p.Arch)
	case "darwin":
		return fmt.Sprintf("libgodotgo-hikawa-darwin-10.6-%s.dylib", p.Arch)
	case "linux":
		return fmt.Sprintf("libgodotgo-hikawa-linux-%s.so", p.Arch)
	default:
